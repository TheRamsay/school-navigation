...
with open("ucebny_ctvrte") as f:
    for room in f.read().split("\n\n"):
        ...
        for name in lines[2:]:
            ...
            if len(parts) == 1:
                try:
                    ...
                    print(f"Error while fetching {parts}")
                    continue
            else:
                try:
                    last_name = parts[0]
        ...
        if not Room.objects.filter(room_id=room_id).exists():
            r = Room.objects.create(room_id=room_id, phone_extension=phone_extension, room_type=room_type, floor=floor,
